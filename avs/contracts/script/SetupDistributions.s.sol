...
contract SetupDistributions is Script, Test {
    ...
    function setUp() public {
        ...
        bondYieldDeployment =
            BondYieldDeploymentLib.readDeploymentJson("deployments/bond-yield/", block.chainid);
        bondYieldConfig =
    ...
    function run() external {
        vm.startBroadcast(bondYieldConfig.rewardsInitiatorKey);
    ...
    function executeProcessClaim() public {
        ...
        vm.startBroadcast(deployer);
        earnerLeaves =
            _getEarnerLeaves(_getEarners(deployer), amountPerPayment, bondYieldDeployment.strategy);
        processClaim(
    ...
    function createAVSRewardsSubmissions(
        ...
        ERC20Mock(bondYieldDeployment.token).mint(
            bondYieldConfig.rewardsInitiator, _amountPerPayment * _numPayments
        );
        ERC20Mock(bondYieldDeployment.token).increaseAllowance(
            bondYieldDeployment.bondYieldServiceManager, _amountPerPayment * _numPayments
        );
        uint32 duration = rewardsCoordinator.MAX_REWARDS_DURATION();
        SetupDistributionsLib.createAVSRewardsSubmissions(
            bondYieldDeployment.bondYieldServiceManager,
            bondYieldDeployment.strategy,
            _numPayments,
            _amountPerPayment,
            duration,
            _startTimestamp
