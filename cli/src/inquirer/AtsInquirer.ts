...
export default class AtsInquirer {
  ...
  public async inquireAddToControlList({
    ...
    try {
      ...
      if (error instanceof Error) {
        console.error(`❌ Error adding to control list: ${error.message}`);
        throw error;
      } else {
        console.error(`❌ Error adding to control list: ${String(error)}`);
        throw new Error(`❌ Error adding to control list: ${String(error)}`);
      }
    }
  }
  ...
  public async inquireRemoveFromControlList({
    ...
    try {
      ...
      if (error instanceof Error) {
        console.error(`❌ Error removing from control list: ${error.message}`);
        throw error;
      } else {
        console.error(`❌ Error removing from control list: ${String(error)}`);
        throw new Error(
