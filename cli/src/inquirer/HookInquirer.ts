    ...
    public async inquireModifyLiquidity({
        ...
        if (!add) {
            try {
                const amount = await input({
                    message:
                        '🔑 Specify how many LP tokens you would like to reclaim: ',
                    required: false,
                    theme: THEME,
                });
                ...
                if (error instanceof Error) {
                    console.error(
                        `❌ Error modifying liquidity: ${error.message}`
                    );
                    throw error;
                } else {
                    console.error(
                        `❌ Error modifying liquidity: ${String(error)}`
                    );
                    throw new Error(
                        `❌ Error modifying liquidity: ${String(error)}`
                    );
            ...
            try {
                ...
                const amount = await input({
                    message:
                        '🔑 Specify how many tokens you would like to deposit.',
                    required: false,
                    theme: THEME,
                });
