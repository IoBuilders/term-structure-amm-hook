...
import {IResolverProxy} from "@hashgraph/ats/contracts/contracts/interfaces/resolver/resolverProxy/IResolverProxy.sol";
import {IFactory} from "@hashgraph/ats/contracts/contracts/interfaces/factory/IFactory.sol";
import {_DEFAULT_ADMIN_ROLE, _ISSUER_ROLE, _CONTROL_LIST_ROLE} from "@hashgraph/ats/contracts/contracts/layer_1/constants/roles.sol";
import {FactoryRegulationData, RegulationType, RegulationSubType} from "@hashgraph/ats/contracts/contracts/layer_3/constants/regulation.sol";
import {BondHookIssuance} from "../src/BondHookIssuance.sol";
import {MockERC20} from "./mock/MockERC20.sol";
import {DECIMAL_PRECISION} from "../src/constants/Constants.sol";
import {IBusinessLogicResolver} from "@hashgraph/ats/contracts/contracts/interfaces/resolver/IBusinessLogicResolver.sol";
import {stdJson} from "forge-std/StdJson.sol";

contract BondDeployer is Script {
    bytes32 constant BOND_CONFIG_ID =
    ...
    function _setBondDeploymentData() internal {
        ...
        IResolverProxy.Rbac memory adminRbac = IResolverProxy.Rbac({
            role: _DEFAULT_ADMIN_ROLE,
            members: admins
        });

        IResolverProxy.Rbac memory issuerRbac = IResolverProxy.Rbac({
            role: _ISSUER_ROLE,
            members: admins
        });

        IResolverProxy.Rbac memory controlListRbac = IResolverProxy.Rbac({
        ...
        bondData.security.resolver = IBusinessLogicResolver(
            businessLogicResolverProxy
        );
        bondData.security.resolverProxyConfiguration = IFactory
            .ResolverProxyConfiguration({key: BOND_CONFIG_ID, version: 0});
    ...
    function _readBondHookDeployment() internal {
        ...
        require(
            vm.exists(fileName),
            "Bond Hook deployment file does not exist"
        );
        string memory json = vm.readFile(fileName);
        hookHub = BondHookHub(
            stdJson.readAddress(json, ".bondHookHub.address")
        );
        manager = PoolManager(
            stdJson.readAddress(json, ".poolManager.address")
        );
    }
    ...
    function _readBlrDeployment() internal {
        ...
        string memory json = vm.readFile(fileName);
        businessLogicResolverProxy = stdJson.readAddress(
            json,
            ".businessLogicResolverProxy.address"
        );
    }
}
